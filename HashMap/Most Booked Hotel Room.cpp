...
Note that this does not mean that all rooms have to be free at the end. In case, 2 rooms have been booked the same number of times, 
return the lexographically smaller room.
...
using namespace std;
auto mostBookedRoom(vector<string> const& bookings) -> string
{
	auto roomToBookings = unordered_map<string, size_t>{};
	...
	for (auto const& booking : bookings)
	{
		...
		++roomToBookings[booking.substr(1)];
	}

	auto mostBooked = pair<string, size_t>{"", 0};

	for (auto const& roomToBookNum : roomToBookings)
	{
		if (roomToBookNum.second > mostBooked.second ||
		    (roomToBookNum.second == mostBooked.second && roomToBookNum.first < mostBooked.first))	//the case where two rooms have the same maximum amount of bookings
		{
			mostBooked = roomToBookNum;
		}
	}

	return mostBooked.first;
}
int main() {
